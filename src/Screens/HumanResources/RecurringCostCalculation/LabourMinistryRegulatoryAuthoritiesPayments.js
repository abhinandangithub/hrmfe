import moment from 'moment'
import React, { useEffect, useState } from 'react'
import { useNavigate  } from 'react-router-dom'
import Button from '../../../Components/Button'
import TableBox from '../../../Components/TableBox/TableBox'
import apiClient from '../../../Util/apiClient'
import { SET_DATA } from '../../../Util/Util'

export default function LabourMinistryRegulatory({ filterData, companyInfo }) {
  const history = useNavigate()
  const [tableData, setTableData] = useState([])

  useEffect(() => {
    const params = {}

    if (filterData.date) {
      params.year = moment(filterData.date).format('YYYY')
      params.month = moment(filterData.date).format('M')
    }

    if (filterData.type === 'Calculate') {
      apiClient
        .post('/recurring-cost-calculations/labour-ministry-regulatory-authorities/generate', params)
        .then(({ data }) => {
          if (data?.result) {
            getData(params)
          }
        })
    } else {
      getData(params)
    }
  }, [filterData])

  const getData = (params = {}) => {
    apiClient
      .get('/recurring-cost-calculations/labour-ministry-regulatory-authorities/get', { params })
      .then(({ data }) => {
        if (data?.result) {
          setTableData(data.result)
        }
      })
  }

  const onGenerateExpense = () => {
    const month = moment(filterData.date).format('MMM')
    const year = moment(filterData.date).format('YYYY')

    const items = tableData.map((v) => ({
      type: '',
      product: 'Labour Ministry Regulatory Authorities Cost',
      description: '',
      quantity: v.noOfVisa,
      unit: 'EA',
      unitPrice: v.visaCost,
      discount: '',
      discountFormat: '%',
      retention: '',
      retentionFormat: '%',
      charge: '',
      chargeFormat: '%',
      tax: 0,
      taxFormat: '%',
      netAmount: '',
      grossAmount: '',
      taxType: 'Normal VAT',
      notes: '',
      account: ''
    }))
    const expenseObj = {
      kind: 'Invoice',
      poNo: `Labour Ministry Regulatory Authorities Cost - ${month} - ${year}`,
      currency: companyInfo?.currency || '',
      issueDate: moment(),
      taxDate: moment(),
      taxCategory: 'Standard',
      items,
      exchangeRate: 1,
      dateOfSupply: moment(),
      paymentTerm: '',
      sentDate: moment(),
      paymentDueDate: '',
      paymentType: '',
      paymentDate: '',
      paidAmount: '',
      status: 'Pending',
      payments: [],

      bank: '',
      bankData: {}
    }
    SET_DATA('expenses.cacheData', expenseObj)
    history('/app/add-expense/Invoice')
  }

  const columns = [
    {
      title: 'Year',
      dataIndex: 'year'
    },
    {
      title: 'Month',
      dataIndex: 'month'
    },
    {
      title: 'No of visa',
      dataIndex: 'noOfVisa'
    },
    {
      title: 'Visa Cost',
      dataIndex: 'visaCost'
    },
    {
      title: 'Total',
      dataIndex: 'amount'
    }
  ]

  return (
    <div>
      <TableBox columns={columns} dataSource={tableData} />
      {filterData.date && tableData.length > 0 && (
        <div style={{ float: 'right', padding: 20 }}>
          <Button variant="primary" onClick={() => onGenerateExpense()}>
            Generate Expense
          </Button>
        </div>
      )}
    </div>
  )
}
