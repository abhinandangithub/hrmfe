...
function Filter(props) {
  ...
  const onSubmitForm = async () => {
    props.validateForm().then((err) => {
      if (_.isEmpty(err)) {
        ...
        SET_DATA('jobSequences.filterData', data)
        props.onFilter(data)
      }
    })
  }
  ...
  return (
    <>
      {validateAccess('add-job-sequence') && (
        <Button onClick={() => history.push('/app/add-job-sequence')} variant="primary" className="btn-block">
          <i className="flaticon-plus" /> Add Sequence
        </Button>
      )}
      <Form>
...
export default withFormik({
  mapPropsToValues: () => {
    const filterObj = GET_DATA('jobSequences.filterData')
