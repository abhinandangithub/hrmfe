...
export default function TerminationChecklists() {
  ...
  function tableAction(param: any) {
    if (param.TYPE === 'EDIT') {
      history.push(`/app/edit-termination-checklist/${param.id}`)
    }
  }

  const DesignationsJson = {
    title: 'Terminaiton Checklist',
    page: 'TERMINATION',
    endpoint: 'termination-checklists/get',
    setFilter: 'terminationChecklists.filterData',
    table: {
      header: [
        {
          flex: 0.25,
  ...
  const emitData = (param: any) => {
    switch (param.TYPE) {
      case 'NEW':
        return history.push('/app/add-termination-checklist')
      default:
        console.log('test')
        break
    }
