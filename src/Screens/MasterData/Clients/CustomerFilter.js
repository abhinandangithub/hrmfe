...
function Filter(props) {
  ...
  const onSubmitForm = async () => {
    props.validateForm().then((err) => {
      if (_.isEmpty(err)) {
        ...
        SET_DATA('customers.filterData', props.values)
      }
    })
  }
  ...
  return (
    <>
      {validateAccess('add-customer') && (
        <Button onClick={() => history.push('/app/add-customer')} variant="primary" className="btn-block">
          <i className="flaticon-plus" /> Add Customer
        </Button>
      )}
      <Form>
...
export default withFormik({
  mapPropsToValues: () => {
    const filterObj = GET_DATA('customers.filterData')
