...
function ClientForm({
  ...
  const getData = () => {
    ...
    apiClient
      .get('options/get-by-types', { params: { type: ['PaymentType', 'VendorGroup'] } })
      .then(({ data }) => {
        if (data && data.result) {
  ...
  useEffect(() => {
    if (values.type) {
      apiClient.get('chart-of-accounts/options/SUPACC').then(({ data }) => {
        if (data?.result) {
    ...
    <Form>
      <Row justify="center" className="inner-contents">
        <Col xs={22} sm={22} md={20} lg={20}>
          <PanelLayout title={t('Vendor Details')}>
            <Row gutter={[10, 10]} className="mb-4">
              ...
              {/* {!id && (
                <Col xs={12} sm={12} md={12} lg={4}>
                  <div className="form-field">
                    <Field label="Add AP Account" name="newAccount" as="select" options={YES_NO_OPTIONS} />
                  </div>
                </Col>
              )} */}
              ...
              <Col xs={12} sm={12} md={12} lg={4}>
                <div className="form-field">
                  <Field label="Group" name="group" as="select" options={options.VendorGroup || []} />
                </div>
              </Col>
              {values.type === 'Vendor' && (
                <Col xs={12} sm={12} md={12} lg={4}>
              ...
              {values.type === 'Vendor' && (
                <Col xs={12} sm={12} md={12} lg={4}>
                  <div className="form-field mt-4 pt-1 mb-3">
                    <Field as="checkbox" name="serviceProvider" label="Is Service Provider" />
                  </div>
                </Col>
              )}
            </Row>
