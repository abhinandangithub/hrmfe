...
class UploadWarehouses extends React.Component {
  constructor() {
    super()
    this.state = {
      uploadData: [],
  ...
  onUpload = (header, rowData) => {
    const { configurations: { warehouseLocations, warehouseRacks } = {} } = this.props.companyInfo

    const validateFields = [
      { label: 'Warehouse', value: 'warehouse' },
      { label: 'Warehouse Description', value: 'description' },
      ...(warehouseLocations && [
      ...
      ...(warehouseRacks && [
        ...
        { label: 'Rack Description', value: 'rackDescription' }
      ])
    ]
    const rows = []
    let errorField = ''
      ...
      header.forEach((col, i) => {
        switch (col) {
          ...
          case 'Warehouse Description':
            obj.description = val[i]
            break
          case 'Location':
            obj.location = val[i]
            break
          case 'Location Description':
            obj.locationDescription = val[i]
            break
          case 'Rack':
            obj.rack = val[i]
            break
          case 'Rack Description':
            obj.rackDescription = val[i]
            break
          default:
            break
        }
  ...
  onUploadData = () => {
    ...
    if (uploadData.length > 0 && !loader) {
      this.setState({ loader: true }, () => {
        masterUpload({ uploadType: 'Warehouses', uploadData }).then((errors) => {
          this.setState({ loader: false, uploadData: [], uploadErrors: errors })
        })
      })
    } else {
      message.error('Please upload warehouses')
    }
  }

  onDowloadTemplate = () => {
    const dowloadUrl = `${API_URL}/assets/defaultTemplate/Warehouse_Template.xlsx`
    const a = document.createElement('a')
    a.href = dowloadUrl
    a.download = 'Warehouse_Template'
    a.click()
  }
                ...
                {this.state.uploadData.length === 0 && (
                  <Col
                    ...
                    <UploadArea
                      ...
                      label="Click here or drag and drop file here to upload warehouses"
                    />
                  </Col>
                )}
                {this.state.uploadData.length === 0 && (
                    ...
                    <Row>
                      <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 24 }} lg={{ span: 24 }}>
                        ...
                        <div className="upload-invoice-expense-note">
                          ...
                          <ul>
                            <li>Upload warehouse not exists in warehouses</li>
                          </ul>
                        </div>
                      </Col>
                    </Row>
...
export default connect(mapStateToProps)(UploadWarehouses)
