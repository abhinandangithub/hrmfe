...
class UploadCategories extends React.Component {
  constructor() {
    super()
    this.state = {
      uploadData: [],
  ...
  onUpload = (header, rowData) => {
    const validateFields = [{ label: 'Name', value: 'name' }]
    const rows = []
    let errorField = ''
  ...
  onUploadData = () => {
    ...
    if (uploadData.length > 0 && !loader) {
      this.setState({ loader: true }, () => {
        masterUpload({ uploadType: 'Categories', uploadData }).then((errors) => {
          this.setState({ loader: false, uploadData: [], uploadErrors: errors })
        })
      })
    } else {
      message.error('Please upload categories')
    }
  }

  onDowloadTemplate = () => {
    const dowloadUrl = `${API_URL}/assets/defaultTemplate/Categories_Template.xlsx`
    const a = document.createElement('a')
    a.href = dowloadUrl
    a.download = 'Categories_Template'
    a.click()
  }
                    ...
                    <Row>
                      <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 24 }} lg={{ span: 24 }}>
                        <div className="upload-invoice-expense-note">
                          ...
                          <ul>
                            <li>Upload category not exists in categories</li>
                          </ul>
                        </div>
                      </Col>
                    </Row>
...
export default connect(mapStateToProps)(UploadCategories)
