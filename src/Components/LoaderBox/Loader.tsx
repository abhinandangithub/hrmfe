...
export function FieldLoader<Props>(loadFn: (props: Props) => Promise<DefaultComponent<Props>>) {
  let height = 60
  ...
  return loadable(loadFn, {
    fallback: (
      <div>
        ...
        <Skeleton.Input active style={{ height: height - 20, borderRadius: 10 }} className="w-100" />
      </div>
    )
  })
}
